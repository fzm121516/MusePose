...
def align_img(img, pose_ori, scales, detect_resolution, image_resolution):
    ...
    scale_sum = 0
    count = 0
    scale_list = [scale_neck, scale_face, scale_shoulder, scale_arm_upper, scale_arm_lower, scale_hand, scale_body_len,
                  scale_leg_upper, scale_leg_lower]
...
def run_align_video_with_filterPose_translate_smooth(args):
    ...
    output_refer, pose_refer = detector(refer_img, detect_resolution=args.detect_resolution,
                                        image_resolution=args.image_resolution, output_type='cv2',
    ...
    for i in range(max_frame):
        ...
        if i == skip_frames:
            output_1st_img, pose_1st_img = detector(img, args.detect_resolution, args.image_resolution,
            ...
            #         count = count + 1
            # if count != 0:
            #     align_args["scale_hand"] = (ratio / count + align_args["scale_arm_upper"] + align_args[
            #         "scale_arm_lower"]) / 3
            # else:
